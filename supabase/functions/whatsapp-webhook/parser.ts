...
export function parseFinancialMessageHeuristic(text: string): ExtractedWhatsAppExpense | null {
  ...
  const amountRegex = /(?:\$|\bars|\busd)?\s*([0-9]{1,3}(?:\.[0-9]{3})*(?:,[0-9]{1,2})?|[0-9]+(?:[.,][0-9]{1,2})?)/i;
...
/**
 * Extracción avanzada usando Gemini Flash Structured Output
 */
export async function parseFinancialMessageWithGemini(
  apiKey: string,
  messageText: string
): Promise<ExtractedWhatsAppExpense | null> {
  try {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;

    const prompt = `Analiza el siguiente mensaje de un usuario en un bot de finanzas personales en español y extrae la información estructurada del movimiento en formato JSON estricto.
Mensaje del usuario: "${messageText}"

Responde UNICAMENTE con un objeto JSON con esta estructura exacta:
{
  "amount": number (monto positivo numérico),
  "currency": "ARS" | "USD",
  "description": string (nombre del comercio o concepto limpio, ej: "Coto", "Farmacity", "Cena con amigos"),
  "categoryHint": string (sugerencia de categoría en español: "Supermercado", "Restaurantes", "Salud", "Transporte", "Servicios", "Entretenimiento", "Ropa", "Educación", "Hogar", "Otros"),
  "accountHint": string o null (nombre de banco o medio de pago si se menciona: "Mercado Pago", "Galicia", "Santander", "BBVA", "Efectivo"),
  "type": "expense" | "income",
  "isTransfer": boolean,
  "installmentInfo": { "current": number, "total": number } o null,
  "confidence": number (entre 0.0 y 1.0)
    ...
    if (!res.ok) {
      console.warn("Gemini API falló, usando parser heurístico de respaldo.");
      return parseFinancialMessageHeuristic(messageText);
    }
    ...
    return {
      ...
      categoryHint: parsed.categoryHint || "Otros",
      accountHint: parsed.accountHint || undefined,
      type: parsed.type === "income" ? "income" : "expense",
      isTransfer: Boolean(parsed.isTransfer),
      installmentInfo: parsed.installmentInfo ? {
        current: parsed.installmentInfo.current || 1,
        total: parsed.installmentInfo.total || 1,
      } : undefined,
      date: new Date().toISOString(),
      confidence: typeof parsed.confidence === "number" ? parsed.confidence : 0.95,
    };
  } catch (err) {
    console.error("Error en parseFinancialMessageWithGemini:", err);
    return parseFinancialMessageHeuristic(messageText);
  }
}
