...
BEGIN
  ...
  VALUES (
    ...
    -- Las tasas internas de la app representan: 1 ARS = X unidades de la divisa (1 / cotización_blue)
    -- Para cotización de 1 USD = 1.300 ARS: USD = 1/1300 (0.00076923)
    -- Para cotización de 1 EUR = 1.420 ARS: EUR = 1/1420 (0.00070422)
    '{"ARS": 1, "USD": 0.000769230769, "EUR": 0.000704225352}'::jsonb
  )
  ON CONFLICT (user_id) DO UPDATE
  ...
  -- 5. Vencimiento Eventual Único (Impuesto Inmobiliario Extraordinario - frequency 'once')
  INSERT INTO public.bill_reminders (
    user_id, name, amount, due_date, frequency, category_id, account_id, status, auto_pay
  ) VALUES (
    p_user_id, 'Tasa Extraordinaria Municipal', 18500, now() + interval '7 days', 'once', v_cat_housing, v_acc_bank_ars, 'pending', false
  );
