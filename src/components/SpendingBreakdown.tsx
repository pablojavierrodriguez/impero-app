...
export function SpendingBreakdown({ transactions }: SpendingBreakdownProps) {
  ...
  return (
    <div className="px-4 py-4">
      ...
      {/* Chart */}
      {chartType !== "none" && chartData.sorted.length > 0 && (
        <div className="mb-4">
          {chartType === "bar" ? (
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={chartData.chartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
            ...
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={chartData.chartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} />
                <Tooltip
                  contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }}
                  formatter={(value: number) => [formatAmount(value), ""]}
      ...
      {chartData.sorted.length > 0 && (
        <div className="flex h-3 rounded-full overflow-hidden mb-4">
          {chartData.sorted.map((cat, i) => (
            <motion.div
              key={cat.name}
              initial={{ width: 0 }}
              animate={{ width: `${(cat.amount / chartData.total) * 100}%` }}
              transition={{ delay: i * 0.05, type: "spring", stiffness: 300, damping: 30 }}
              className={`${cat.color} ${i === 0 ? "rounded-l-full" : ""} ${i === chartData.sorted.length - 1 ? "rounded-r-full" : ""}`}
            />
          ))}
        </div>
      )}
